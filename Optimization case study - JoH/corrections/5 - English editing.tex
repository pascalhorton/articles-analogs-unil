...
\section{Optimization of circulation analogy}
...
The chosen method for the atmospheric circulation analogy, based on four circulation predictors (geopotential heights), and which is named 4Zo (o for optimized), was based on the following degrees of freedom:
...
\subsection{Results for 4Zo method}
...
The resulting spatial windows (Figure \ref{fig:spatial_windows_4Zo}) may look very diverse first, but there are significant similarities for subregions located within the same vicinity. The first four subregions were characterized by a large spatial window on the upper level, whereas it was smaller elsewhere. For most subregions, the second level (Z700) was compared on thin and longitudinally extended spatial windows. The third level (Z1000 at 6 or 12~h) also had longitudinally extended domains, but were slightly larger. The last one (Z1000 at 30~h) had rather large and squared windows. Subregions number 5 (southern valleys) and 6 (southern ridges) had exactly the same spatial windows, which suggests that they behave in a similar way and thus could have been merged. This similarity is a good sign for the accuracy of the optimized parameters.
...
\section{Optimization of analogy with moisture information}
...
\subsection{Results for 4Zo-2MIo method}
...
\begin{itemize}
	...
	\item The automatic and joint optimization of all parameters: the analogue numbers, selection of pressure levels and temporal windows, and spatial windows. These parameters are highly interdependent, so one needs to optimize them jointly in order to identify optimal combinations. Indeed, there is a strong interdependence between space and time in the atmospheric circulation, so that, e.g. the spatial window should move upstream in the main atmospheric flow for earlier temporal windows. 
...
GAs provided parameterizations of AMs that exceeded the performance of sequential calibration. In addition, it has been observed that the prediction for days with strong precipitation were improved to a greater extent, which is clearly interesting in the context of flood forecasting.
...
\begin{figure}[t]
	...
	\caption{Location of alpine Rh\^{o}ne catchment in Switzerland. (source: Swisstopo)}
	\label{fig:map}
\end{figure}
...
\begin{figure}[t]
	...
	\caption{Optimized spatial windows for 4Zo method (analogy of atmospheric circulation on four pressure levels).}
	\label{fig:spatial_windows_4Zo}
\end{figure}

\begin{figure}[t]
	\centerline{\includegraphics[width=8.8cm]{figures/fig04.pdf}}
	\caption{Performance score (CRPSS) of reference methods 2Z (Table \ref{table:params_R1}) and optimized 4Zo method for CP and VP for every subregion.}
...
\begin{figure}[t]
	...
	\caption{Optimized spatial windows for 4Zo-2MIo method (analogy of atmospheric circulation on four pressure levels and analogy on the moisture index on two pressure levels).}
	\label{fig:spatial_windows_4Zo-2MIo}
\end{figure}

\begin{figure}[t]
	\centerline{\includegraphics[width=8.8cm]{figures/fig07.pdf}}
	\caption{Performance score (CRPSS) of reference methods 2Z-2MI (Table \ref{table:params_R2}) and optimized 4Zo-2MIo method for CP and VP for every subregion.}
	\label{fig:figure_crpss_4Zo-2HIo}
\end{figure}

\begin{figure}[t]
	\centerline{\includegraphics[width=8.8cm]{figures/fig08.pdf}}
	\caption{Optimal numbers of analogues for different regions and the two methods, resulting from optimization. Method 4Zo is made of a single level of analogy with $N_{1}$ analogues, whereas 4Zo-2MIo has two levels of analogy with $N_{1}$ and $N_{2}$ analogues.}
...
\begin{figure}[t]
	...
	\caption{Distribution of optimal weights for predictors of first level of analogy (geopotential heights) of (blue) 4Zo, (red) 4Zo-2MIo, and (green) 4Zo-4MIo methods. Results are for the ten subregions. Geopotential heights are sorted by increasing pressure and hour.}
	\label{fig:levels_weights_average}
\end{figure}

\begin{figure}[t]
	\centerline{\includegraphics[width=7.5cm]{figures/fig11.pdf}}
	\caption{Example of evolution of performance score of the best individual over eight independent optimizations.}
	\label{fig:evolution}
\end{figure}
...
\begin{table}[t]
	\caption{Parameters of reference method on atmospheric circulation (2Z). First column is level of analogy (0 for preselection); subsequent columns list meteorological variable, its hour of observation within the target day (temporal window), criterion used for current level of analogy, and number of analogues.}
	\footnotesize
	\begin{center}
		\begin{tabular}{ccccc}
			\hline
...
\begin{table}[t]
	\caption{Parameters of reference method with moisture variables (2Z-2MI). Conventions are the same as in Table \ref{table:params_R1}}
	\footnotesize
	\begin{center}
		\begin{tabular}{ccccc}
			\hline 
...
\begin{table}[t]
	\caption{Pressure levels ($\sim$) automatically selected for 4Zo method for different subregions (ID). R represents 2Z reference method (Table \ref{table:params_R1}).}
	\footnotesize
	\begin{center}
...
\begin{table}[t]
	\caption{Relative improvement (\%) in CRPSS for different precipitation thresholds for optimized 4Zo method, compared to reference method.}
	\footnotesize
	\begin{center}
...
\begin{table}[t]
	\caption{Atmospheric levels automatically selected for analogy of atmospheric circulation ($\sim$) and moisture analogy ($\bullet$) of 4Zo-2MIo method, for different subregions (ID). R represents 2Z-2MI reference method (Table \ref{table:params_R2})}
	\footnotesize
	\begin{center}
		\begin{tabular}{ccccccccc}
			\hline ID & 300 & 400 & 500 & 600 & 700 & 850 & 925 & 1000 \\ 
...
\begin{table}[t]
	\caption{Relative improvement (\%) in CRPSS for different precipitations thresholds for optimized 4Zo-2MIo method, compared to reference method.}
	\footnotesize
	\begin{center}
		\begin{tabular}{ccccccc}
			\hline 
