...
The AM relies on two types of data: predictors, that are atmospheric variables describing the state of the atmosphere at a synoptic scale, and the predictand, which is the local weather variable one wants to predict.

Predictors are generally extracted from reanalysis datasets. NCEP-NCAR reanalysis \citep[6-hourly, 17 pressure levels at a resolution of 2.5\degree, see][]{Kalnay1996} is used here, but it could be any other reanalysis dataset.
...
The number of predictor was then successively increased up to ten, considering the weighting of the analogy criteria values. The addition of predictors constantly (for both the CP and the VP) improved the prediction skill only up to four predictors (Figure \ref{fig:figure_nb_levels}). Afterwards, the score on the VP is more variable, showing eventually even a decrease, which revealed an over-parametrization of the method, and thus a lack of robustness. After four predictors, the score on the CP does not increase substantially, and might even present some local decrease due to increasing difficulty for the optimizer to converge. Considering four predictors is considered optimal for this case study, since the gain in CRPSS is significant, and the model remains relatively simple. It cannot be excluded that another number would prevail in another region than the upper Rh\^{o}ne catchment, under other meteorological conditions, or with another reanalysis dataset.

\subsection{Which parameters are optimized ?}

The chosen method for the atmospheric circulation analogy, based on four levels of the geopotential, and that will be named 4Zo, is made of the following degrees of freedom:
...
\begin{itemize}
	\item Using four pressure levels for the analogy of circulation seems to be an optimal number for the studied region, length of archive available, and target predictand considered. Beyond four, the validation score is more variable, revealing a loss in robustness due to over-parametrization.
