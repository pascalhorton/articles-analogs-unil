...
The most common usage of the analogue method is for precipitation forecasting. Thus, the evolution presented hereafter focuses on this predictand.
...
\citet{BenDaoud2010} applied the analogue method in the context of large flatter catchments, out of an Alpine environment (Sa\^{o}ne, Seine). He evaluated several variables usually used in weather forecasting, amongst them the air temperature and the vertical velocity that presented a potential interest. The temperature (at the nearest grid point, on the 925~hPa (36~h) and 600~hPa (12~h) levels) could replace the rigid selection of $\pm$ 60 days around the target date by a more dynamic screening of similar situations in terms of air masses. The seasonal effect is indeed also present in the temperature data. The vertical velocity was used as an additional level of analogy, between the analogy of atmospheric circulation and the one on humidity, to account for fronts. Finally, he selected the humidity variables at 700~hPa instead of 850~Pa. 
...
\begin{enumerate}
	...
	\item Subsequent level(s) of analogy: we can then subsample once again the $N_{1}$ analogues on the basis of another variable in order to obtain a lower number of analogue dates, $N_{2}$. The S1 criterion is usually not relevant for other predictors that the atmospheric circulation. Other classic criteria represent absolute distances: Mean Absolute Error (MAE) and Root Mean Squared Error (RMSE), the latest being most often used. These are more relevant to compare humidity variables for example.
...
\begin{itemize}
	...
	\item HI : humidity index (TPW * RH)
	\item HF : humidity flux (V * TPW * RH)
...
\subsection{References for precipitation forecasting}

The evolution of the analogue method for precipitation forecasting led to several improvements. However, some of these are specific for a certain region and are not relevant for others, and some perform better depending on the lead time. Thereby, we cannot provide a single best parameterization for the method, neither rank them from worst to best. We will thus provide the references implementations of the most recent method variant, in the chronological order they appeared.

The first parameterization comes from the work of \citet{Guilbaud1997} and \citet{Obled2002}. It consists of a single level of analogy on the atmospheric circulation. The score S1 (Teweles-Wobus, Eq. \ref{eq:S1}) is processed on the geopotential height at 1000~hPa (Z1000) and 700~hPa (Z700) (Table \ref{table:R0a}) or 500~hPa (Z500) (Table \ref{table:R0b}), both at 00~h and 24~h. The number of analogues considered is 50.

\begin{table}[htbp]
	\caption{Method of type PC-4Z proposed by \citep{Guilbaud1997}.}
	\begin{tabular}{ccccc}
		\tophline
		...
		\middlehline 
		\multirow{2}{*}{1} & Z700 & 00, 24~h & \multirow{2}{*}{S1} & \multirow{2}{*}{50} \\
		& Z1000 & 00, 24~h & & \\ 
		\bottomhline 
	\end{tabular} 
	\label{table:R0a}
...
\begin{table}[htbp]
	\caption{Variant of the PC-4Z method proposed by \citep{Guilbaud1997}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline 
		0 & \multicolumn{4}{l}{$\pm 60$ days around the target date} \\
		\middlehline 
		\multirow{2}{*}{1} & Z500 & 00, 24~h & \multirow{2}{*}{S1} & \multirow{2}{*}{50} \\
		& Z1000 & 00, 24~h & & \\ 
		\bottomhline 
	\end{tabular} 
	\label{table:R0b}
...
%\subsection{R1, analogie de circulation}
%\label{sec:etat_art:R1}
%\review{6}

%Ce que nous considérerons comme notre première référence est l'analogie de circulation (R1, Table \ref{table:R1}) optimisée par \citet{Bontron2004}. Ce dernier a observé que le choix de l'heure était plus important que celui du niveau atmosphérique. La méthode est alors constituée du géopotentiel à deux niveaux atmosphériques et à des heures différentes.

\begin{table}[htbp]
	\caption{Method of type PC-2Z proposed by \citep{Bontron2004}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline 
...
%\subsection{R2, prise en compte de l'humidité}
%\label{sec:etat_art:R2}
%\review{6}

%Toujours d'après les travaux de \citet{Bontron2004}, la référence suivante (R2, Table \ref{table:R2}) intègre un second niveau d'analogie sur un indice d'humidité: le produit de l'humidité relative à 850~hPa et de l'eau précipitable de la colonne. Cette variable ne représente pas une grandeur physique réelle, mais exprime le contenu en eau de la colonne et sa proximité ou son éloignement de la saturation (à 850~hPa).

\begin{table}[htbp]
	\caption{Method of type PC-2Z-2HI proposed by \citep{Bontron2004}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline 
...
%\subsection{R3, prise en compte du flux d'humidité}
%\label{sec:etat_art:R3}

%\citet{Marty2010} a recalibré les paramètres déterminés par \citet{Bontron2004} pour une application infrajournalière (résolution temporelle de 12~h), et sur la base d'un jeu de données de réanalyses plus complet. Il a proposé deux modèles avec un second niveau d'analogie sur l'humidité (R3a, Table \ref{table:R3a}) ou le flux d'humidité au niveau 700~hPa (R3b, Table \ref{table:R3b}) ou 925~hPa (R3c, Table \ref{table:R3c}). Les heures et les niveaux changent par rapport à ceux établis par \citet{Bontron2004}, car ceux-ci n'étaient alors pas disponibles.


\begin{table}[htbp]
	\caption{Method of type PC-2Z-2HI proposed by \citep{Marty2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline 
...
\begin{table}[htbp]
	\caption{Method of type PC-2Z-2HF proposed by \citep{Marty2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline 
		...
		& Z 500~hPa & 18~h & & \\ 
		\middlehline
		2 & V700 * TPW * RH700 & 06, 18~h & RMSE & 25 \\
		\bottomhline 
	\end{tabular} 
	\label{table:R3b}
\end{table}
...
\begin{table}[htbp]
	...
	\begin{tabular}{ccccc}
		...
		& Z 500~hPa & 18~h & & \\ 
		\middlehline
		2 & V925 * TPW * RH925 & 06, 18~h & RMSE & 25 \\
		\bottomhline 
	\end{tabular} 
	\label{table:R3c}
\end{table}
...
%Le flux d'humidité est composé du produit de l'eau précipitable et de l'humidité relative, multiplié par la norme de la vitesse du vent ($V$).


%\subsection{R4, présélection sur les températures}
%\label{sec:etat_art:R4}
%\review{6}

%La quatrième référence reconsidère la sélection des journées candidates à l'analogie et utilise la présélection sur les températures introduite par \citet{BenDaoud2010}. Nous nous affranchissons donc de la sélection d'une période de 4 mois centrée sur la date pour une approche plus dynamique qui tient différemment compte de l'effet de saisonnalité. Le nombre d'analogues de la présélection est équivalent au nombre de jours que nous aurions retenus par la période de $\pm 60$ jours, soit $N_{0} = 60 \cdot 2 \cdot n_{a}$, où $n_{a}$ est le nombre d'années de la période de calibration. Mais la sélection est ici fondée sur un critère de similitude thermique au droit de la région cible.

\begin{table}[htbp]
	\caption{Method of type P2T-2Z-2HI proposed by \citep{BenDaoud2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline
...
\begin{table}[htbp]
	\caption{Method of type P2T-2Z-4HI proposed by \citep{BenDaoud2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline
		...
		& Z 500~hPa & 24~h & & \\ 
		\middlehline
		\multirow{2}{*}{2} & TPW * RH925 & 12, 24~h & \multirow{2}{*}{RMSE} & \multirow{2}{*}{25} \\
		& TPW * RH700 & 12, 24~h & & \\ 
		\bottomhline 
	\end{tabular} 
	\label{table:R4b}
...
\begin{table}[htbp]
	\caption{Method of type P2T-2Z-10HI proposed by \citep{BenDaoud2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline
		...
		& Z 500~hPa & 24~h & & \\ 
		\middlehline
		\multirow{2}{*}{2} & TPW * RH925 & 06-30~h & \multirow{2}{*}{RMSE} & \multirow{2}{*}{25} \\
		& TPW * RH700 & 06-30~h & & \\ 
		\bottomhline 
	\end{tabular} 
	\label{table:R4c}
...
%Trois références sont donc retenues ici: la première (R4a, Table \ref{table:R4a}) considère le niveau d'analogie sur l'humidité tel que l'a défini \citet{Bontron2004}, la deuxième (R4b, Table \ref{table:R4b}) intègre les niveaux d'humidité calibrés par \citet{BenDaoud2010}, et la troisième (R4c, Table \ref{table:R4c}) considère toutes les heures d'observation disponibles pour ce dernier niveau d'analogie.


%\subsection{R5, prise en compte des mouvements verticaux}
%\label{sec:etat_art:R5}

%\citet{BenDaoud2010} a encore ajouté un niveau d'analogie sur les mouvements verticaux. Ce niveau vient s'insérer entre l'analogie de circulation et celle de l'humidité. À nouveau, nous considérons une première référence (R5a, Table \ref{table:R5a}) avec les paramètres d'humidité selon \citet{Bontron2004}, et une seconde (R5b, Table \ref{table:R5b}) avec les paramètres de \citet{BenDaoud2010}.

\begin{table}[htbp]
	\caption{Method of type P2T-2Z-4W-2HI proposed by \citep{BenDaoud2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline
...
\begin{table}[htbp]
	\caption{Method of type P2T-2Z-4W-4HI proposed by \citep{BenDaoud2010}.}
	\begin{tabular}{ccccc}
		\tophline
		\textbf{Level} & \textbf{Variable} & \textbf{Hour} & \textbf{Criteria} & \textbf{Nb} \\ 
		\middlehline
...
\begin{table}[htbp]
	\caption{Method of type PC-4Zo proposed by \citep{Horton2012}.}
	\begin{tabular}{ccccc}
...
\begin{table}[htbp]
	\caption{Method of type PC-4Zo-2HIo proposed by \citep{Horton2012}.}
	\begin{tabular}{ccccc}
...
%\citet{Djerboua2001} a également proposé de normaliser la pluie par la pluie décennale à la station (ou groupement), ce qui nous permet de mieux identifier les contributions les plus importantes en précipitations.

%Par une étude de sensibilité, \citet{Bontron2004} a déterminé un plafonnement de la longueur de l'archive nécessaire à 30 ans pour la prévision en général, et à 40 ans pour les événements intenses. Il a également mis en lumière la dépendance entre le nombre d'analogues optimal et la taille de l'archive: plus l'archive est courte, plus le nombre d'analogues diminue. En faisant varier la résolution des grilles de prédicteurs, il en a conclu que l'augmentation en performance n'est pas significative pour l'analogie de circulation en deçà d'une résolution de 5\degres . Cependant, elle semble plus importante pour le second niveau d'analogie sur les humidités, qui sont des variables plus locales.

%\citet{BenDaoud2010} a dans un premier temps évalué les réanalyses européennes ERA-40 \citep{Uppala1999}, avec une résolution de 1.125\degres, par rapport aux réanalyses américaines du NCEP/NCAR à 2.5\degres\ \citep{Kalnay1996} utilisées jusqu'alors. Il a identifié une légère amélioration des performances, mais pas assez substantielle pour vraiment justifier un changement d'archive \citep{BenDaoud2008}.

\subsection{Discussion}
% TODO: write

%\citet{Bliefernicht2010} a observé que la performance de la méthode est plus élevée en hiver qu'en été. Ainsi, la relation entre les prédicteurs synoptiques et les précipitations locales est plus faible en été, en raison des pluies d'origine convective qui ont un caractère plus aléatoire.

%La méthode des analogues, ou une forme proche, a également été utilisée pour la prévision à court ou moyen terme des températures journalières \citep{Radinovic1975, Woodcock1980, Kruizinga1983}, des vents \citep{Gordon1987}, des avalanches \citep{Obled1980, Bolognesi1993}, de l'insolation \citep{Bois1981}, et de la trajectoire de cyclones tropicaux \citep{Keenan1981, Sievers2000, Fraedrich2003}.

%\citet{Guilbaud1997} a effectué une recherche bibliographique des utilisations de la méthode en prévision à long terme et a identifié des applications opérationnelles pour des prévisions mensuelles dans de nombreux pays, notamment au Canada \citep{Shabbar1986}, en Hongrie \citep{Toth1989}, aux Pays-Bas \citep{Nap1981}, et en Angleterre \citep{Murray1974}, ainsi que des prévisions saisonnières : \citet{Barnett1978}, \citet{Bergen1982} et \citet{Livezey1988}.

%L'utilisation de la méthode des analogues en tant que technique d'adaptation de modèles climatiques fait l'objet d'un nombre croissant d'études \citep{Zorita1999,Wetterhall2005,Matulla2008}. \citet{Wetterhall2007} ont également utilisé la méthode des analogues en Suède, dans le cadre d'une étude sur le réchauffement climatique, et la trouvent simple, mais moins performante que d'autres méthodes plus complexes. Cependant, ils n'utilisent que la pression au niveau de la mer en tant que prédicteur, et considèrent uniquement la meilleure analogue. Ces deux aspects ne sont clairement pas optimaux, et une qualité médiocre de la méthode n'est alors pas surprenante. L'implémentation actuelle de la méthode (voir section \ref{sec:etat_art:implementation_actuelle}) fournit clairement des résultats plus pertinents.

%Une version de la méthode a été évaluée dans le projet STARDEX \citep[\textit{STAtistical and Regional dynamical Downscaling of EXtremes for European regions}, voir][]{Goodess2003, Stardex2005}. Un des objectifs du projet étant de comparer différentes méthodes de descente d'échelle pour la détermination des extrêmes météorologiques, la méthode des analogues a été mise en concurrence avec d'autres techniques et a été sélectionnée parmi les plus intéressantes \citep{Maheras2005, Schmidli2007}.

%\citet{GibergansBaguena2007} ont implémenté la méthode pour des prévisions de précipitations journalières en Catalogne. Ils ont considéré le géopotentiel à 700 et 1000~hPa, ainsi que l'épaisseur entre ces deux niveaux pour la recherche d'analogie de circulation, puis ont appliqué une seconde sélection sur la base de données locales obtenues par radiosondage: indices de stabilité, eau précipitable, température, etc...

%\citet{Hamill2006} ont utilisé une approche par analogie sur les reprévisions du modèle global des États-Unis (GFS, voir sections \ref{sec:donnees:previsions_GFS} et \ref{sec:donnees:resimulations_GFS}). Cette application a permis de corriger des erreurs systématiques dans les prévisions d'ensemble des températures et des précipitations. En effet, l'approche statistique a mis en évidence certains biais dans les prévisions du modèle numérique, qui ont pu être ajustés par la prise en compte de la climatologie locale intrinsèque à la méthode des analogues. De plus, la dispersion des analogues étant plus importante que celle des prévisions d'ensemble, la sous-dispersion de ces dernières a pu être corrigée à l'aide des analogues \citep{Hamill2006}. Une des raisons invoquées pour la bonne performance de la méthode des analogues par rapport au modèle numérique global est le fait que ce dernier a une orographie lissée, qui ne tient pas compte des réelles spécificités locales, ce que permet l'approche par analogie.

%La méthode présente cependant des limites dont il faut être conscient. La première est la nécessité de disposer d'une archive conséquente de la variable cible à un pas de temps pertinent, ici les précipitations. Sans une base de données de prédictands de plusieurs décennies, la méthode n'est pas applicable.

%Les archives des prédicteurs sont également importantes et plus particulièrement leur homogénéité. Nous disposons actuellement de jeux de données de réanalyses (voir section \ref{sec:donnees:reanalyses1_ncep}) de bonne qualité, mais contenant tout de même des discontinuités significatives dues à la densification des réseaux de mesures et au déploiement de nouvelles techniques de mesure et de l'imagerie satellitaire. Nous ne pourrons a priori jamais complètement éliminer cette hétérogénéité, car les données demeureront manquantes. Toutefois, la qualité de ces données les rend tout à fait utilisables pour la méthode des analogues, et ceci partout sur le globe !

%Un aspect supplémentaire concerne les différences entre les prédicteurs disponibles en temps réel et les archives de ces prédicteurs. En effet, les deux jeux de données n'ont généralement pas la même climatologie, car ils ne résultent pas du même modèle, et n'ont pas été élaborés de la même manière. Il existe des alternatives, telles que l'utilisation en opérationnel de prévisions émises par un modèle figé, conjointement à des archives élaborées par ce même modèle en reprévision. 


%Toutefois, les prévisions que nous utiliserions de cette manière ne seraient pas à l'état de l'art, et les reprévisions (archives) contiendraient des erreurs et des incertitudes beaucoup plus importantes que des réanalyses. Il convient donc d'utiliser des prédicteurs robustes qui dépendent au minimum des spécificités du modèle numérique.

%Une autre limite réside dans le fait que les événements extrêmes peuvent être sous-représentés dans l'échantillon d'analogues considéré. En effet, dans une archive météorologique d'une durée limitée, les événements correspondants à des temps de retour élevés ne sont pas très nombreux. Leur représentation est certainement inférieure au nombre d'analogues que nous retenons, ce qui peut biaiser la prévision.

%Enfin, \citet{Bliefernicht2007} ont montré que les prévisions sont moins bonnes en été. Ceci est dû à la part plus importante de la convection durant les mois chauds. Les variables de la circulation synoptique ne sont en effet pas à même de prévoir la localisation des cellules orageuses. Ceci a également été constaté par \citet{BenDaoud2010}, qui avait mis en place un modèle spécifique pour les mois d'été (du 15 juin au 15 septembre). Nous pouvons estimer que les prédicteurs sont à reconsidérer (variables, niveaux, etc.), et que leur domaine peut être différent. Le problème est que, durant cette période, on peut également rencontrer situations classiques avec une circulation atmosphérique dominante comme dans les autres saisons, ou des situations de marais barométriques accompagnées d'un fort risque de convection locale, sans signature dans les variables synoptiques.
...
%\citet{Djerboua2001} a appliqué la méthode développée par \citet{Guilbaud1997} en prévision opérationnelle dans le cadre du projet MAP \citep[\textit{Mesoscale Alpine Programme}, voir][]{Binder1996}, lors de la période spéciale d'observation \citep{Bougeault2001}. Il a relevé que le quantile 60~\% prévoyait le mieux les quantités de précipitations pour les situations normales, mais que lorsque la pluie observée était forte à extrême, le quantile 90~\% s'avérait être un meilleur témoin. Il convient donc de porter une attention particulière au quantile 90~\% qui, lorsqu'il monte brutalement, peut être le signe d'une situation propice à un événement de nature extrême, par la présence de plusieurs analogues dans la distribution avec des cumuls observés significatifs \citep{Djerboua2001}. En opérationnel, les prévisionnistes doivent prêter attention à la distribution des meilleures analogues. Si, par exemple, celle-ci est décalée vers des valeurs plus élevées, il y a un risque de sous-estimer l'événement.

%En comparant les performances des modèles de Météo France avec la méthode des analogues, il en a conclu que les modèles numériques sont meilleurs pour la prévision du jour même, alors que les analogues deviennent plus concurrentielles (et meilleures pour les bassins de taille moyenne) pour les échéances suivantes.

% En opérationnel, ce second niveau est pertinent jusqu'à une échéance de 3~jours. Au-delà, les prévisions d'humidité ne sont pas assez performantes pour être utiles.

%En analysant les distributions de précipitations issues des analogues, \citet{Bontron2004} a identifié le quantile 60~\% comme étant le plus informatif pour l'occurrence des précipitations, et le quantile 90~\% pour la prévision des précipitations moyennes à fortes ($P > P10/10$, soit un dixième de la pluie décennale).

%En opérationnel, ces niveaux supplémentaires se sont montrés utiles pour les deux à trois premières échéances, mais au-delà, toutes les méthodes deviennent équivalentes à l'analogie sur la circulation, car les autres variables, comme la vitesse verticale, sont alors mal prévues.
