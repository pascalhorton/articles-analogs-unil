...
The next reference method was developed by \citet{BenDaoud2010} and introduces a change in the preselection. The calendar selection ($\pm 60$ days around the target date) was then replaced by a preselection based on air temperature at 925~hPa (36~h) and 600~hPa (12h). The number of preselected dates is equivalent to the number of days we would have chosen with the calendar approach, and thus depends on the archive size: $N_{0} = 60 \cdot 2 \cdot n_{a}$ where $n_{a}$ is the number of years in the calibration period. This analogy on air temperature is performed at the closest point to the catchment

\citet{BenDaoud2010} also reconsidered the parameters of the moisture index and ended up with both 925~hPa and 700~hPa levels, at 12~h and 24~h, or at every time step (6~h) between 06~h and 30~h (Table \ref{table:R4b}).
