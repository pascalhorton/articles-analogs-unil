...
The Viewer module allows displaying the resulting forecast files in an interactive GIS environment (Figure \ref{figure:atmoswing-viewer-gui}). 
...
Automatically loads the most recent forecasts
GIS layers
Workspace
Alarm panel
...
%
%L'outil de visualisation présente plusieurs niveaux de synthèse des prévisions. L'utilisateur peut ainsi passer d'un aperçu très global des tendances au détail des analogues à la station. Les différents niveaux d'agrégation, du plus général au plus détaillé, sont les suivants:
%
%\begin{itemize}\addtolength{\itemsep}{0.5\baselineskip}
%	\vspace{2mm}
%	\item \textbf{Le panneau des alarmes}: après ouverture des fichiers de prévision, un panneau latéral (voir Figure \ref{fig:Atmoswing_viewer}) présente une grille synthétisant tous les modèles (lignes) et toutes les échéances (colonnes). La couleur affichée dans une cellule de la grille, correspondant donc à un modèle (paramétrisation de la méthode des analogues) et une échéance, est la valeur maximale prévue de toutes les stations (pour un temps de retour et un quantile choisi par l'utilisateur); ce panneau permet d'identifier en un coup d'?il si une situation critique est prévue pour au moins une station par un modèle et pour une échéance.
%	
%	\item \textbf{La carte des échéances}: la première carte proposée à l'utilisateur est une synthèse des prochaines échéances pour un modèle donné (Figure \ref{fig:Atmoswing_carte_echeances}). Nous avons conçu une représentation symbologique permettant un affichage synthétique: à l'emplacement de chaque station, une bande circulaire est affichée avec un casier par échéance. 
%	
%	\newpage
%	\vspace*{-30mm}
%	
%	Le nombre de casiers est ajusté au nombre d'échéances. La couleur attribuée à chaque casier est proportionnelle à la valeur prévue pour l'échéance en question. Cette représentation offre un rapide aperçu des jours à venir et permet d'identifier facilement un éventuel événement prévu par une paramétrisation des analogues. Elle offre une visualisation spatio-temporelle globale avant d'entrer davantage dans les détails du modèle. L'utilisateur peut passer d'un modèle à un autre en les sélectionnant dans la barre latérale (voir Figure \ref{fig:Atmoswing_viewer}). Il peut également ajuster la représentation des prévisions en choisissant une normalisation par un des nombreux temps de retour proposés, et en sélectionnant un quantile spécifique (20~\%, 60~\% ou 90~\%).
%	
%	\item \textbf{La carte journalière}: l'utilisateur peut ensuite afficher la carte pour une échéance du modèle (voir Figure \ref{fig:Atmoswing_carte_echeance_unique}) afin de mieux visualiser la spatialisation des précipitations et d'accéder directement aux valeurs attendues (affichées à chaque station). Un curseur situé sur le haut de la carte permet de changer d'échéance.
%	
%	\item \textbf{Les séries temporelles}: lorsqu'une station est sélectionnée (sur la carte ou dans la liste déroulante de la barre latérale), le graphique de la série temporelle prévue s'affiche (Figure \ref{fig:Atmoswing_serie_temporelle}). Les options d'affichage sont les suivantes:
%	\begin{itemize}
%		\item les 3 quantiles classiques (20~\%, 60~\%, 90~\%) sous forme de lignes bleues épaisses;
%		\item tous les dixièmes de quantiles par des surfaces grisées;
%		\item toutes les analogues sous forme de petites croix grises;
%		\item les 5 ou 10 meilleures analogues par des croix jaunes, oranges, ou rouges. Les couleurs sont proportionnelles aux rangs des analogues. Le rouge est pour la meilleure analogue et le jaune pour la 5\ieme\ ou 10\ieme\ ;
%		\item la valeur du temps de retour de 10 ans, ou de tous les temps de retour, sous la forme de traits rouges;
%		\item les traces des anciennes prévisions, afin d'évaluer la cohérence temporelle.
%	\end{itemize}
%	
%	\item \textbf{Les distributions}: pour une station et une date choisie, l'utilisateur peut visualiser les distributions du critère d'analogie (Figure \ref{fig:Atmoswing_distrib_critere}) et des précipitations (Figure \ref{fig:Atmoswing_distrib_precip}). La distribution du critère d'analogie permet d'identifier d'éventuelles discontinuités dans les valeurs des critères et donc de porter plus de crédit aux meilleures analogues. Le graphique des précipitations permet de comparer la distribution de toutes les analogues et celle des 5 ou 10 meilleures.
%	
%	\item \textbf{Le détail des analogues}: le détail des dates analogues, des valeurs du critère et des précipitations associées peut être affiché dans un tableur interactif (Figure \ref{fig:Atmoswing_liste_analogues}).
%	
%\end{itemize}
%
%\vfill\vfill\vfill
...
%			\includegraphics[width=10cm]{Atmoswing_carte_echeances.jpg}
%		\end{minipage}
%		\hspace{0.5cm}
%		\begin{minipage}[c]{3.8cm}
%			\includegraphics[width=3.8cm]{Atmoswing_bande_echeances.pdf}
%		\end{minipage}
%	\end{center}
%	\vspace{-2mm}
%	\caption[Carte avec toutes les échéances, et symbole élaboré à cette fin.]{Carte avec toutes les échéances, et symbole élaboré à cette fin.}
%	\label{fig:Atmoswing_carte_echeances}
%\end{figure}
%
%\vfill\vfill\vfill
%
...
%	\vspace*{2mm}
%	\begin{center}
%		\includegraphics[width=10cm]{Atmoswing_carte_echeance_unique.jpg}
%	\end{center}
%	\vspace{-2mm}
%	\caption[Carte pour une échéance.]{Carte pour une échéance.}
%	\label{fig:Atmoswing_carte_echeance_unique}
%\end{figure}
%
%\begin{figure}[htbp]
%	\vspace*{2mm}
%	\begin{center}
%		\includegraphics[width=10cm]{Atmoswing_serie_temporelle.jpg}
%	\end{center}
%	\vspace{-2mm}
%	\caption[Graphique de l'évolution temporelle pour une station.]{Graphique de l'évolution temporelle pour une station.}
%	\label{fig:Atmoswing_serie_temporelle}
%\end{figure}
%
%\begin{figure}[htbp]
%	\vspace*{2mm}
%	\begin{center}
%		\includegraphics[width=10cm]{Atmoswing_distrib_critere.jpg}
%	\end{center}
%	\vspace{-2mm}
%	\caption[Graphique de la distribution du critère d'analogie des journées analogues.]{Graphique de la distribution du critère d'analogie des journées analogues.}
%	\label{fig:Atmoswing_distrib_critere}
%\end{figure}
%
%\begin{figure}[htbp]
%	\vspace*{2mm}
%	\begin{center}
%		\includegraphics[width=10cm]{Atmoswing_distrib_precip.jpg}
%	\end{center}
%	\vspace{-2mm}
%	\caption[Graphique de la distribution des valeurs de précipitations des journées analogues.]{Graphique de la distribution des valeurs de précipitations des journées analogues.}
%	\label{fig:Atmoswing_distrib_precip}
%\end{figure}
%
%\begin{figure}[htbp]
%	\vspace*{2mm}
%	\begin{center}
%		\includegraphics[width=9cm]{Atmoswing_liste_analogues.jpg}
%	\end{center}
%	\vspace{-2mm}
%	\caption[Liste des journées analogues et de leurs précipitations associées.]{Liste des journées analogues et de leurs précipitations associées.}
%	\label{fig:Atmoswing_liste_analogues}
%\end{figure}
%
%
%Lors de la calibration (section \ref{sec:implementation:calibration}), nous avons déterminé des paramètres de la méthode des analogues optimaux pour certaines régions. Ces paramètres sont donc certainement plus robustes que s'ils avaient été établis directement à la station. Toutefois, lors du calcul de prévision, nous appliquons ces paramètres à l'échelle de la station individuelle. Ce choix a été fait pour offrir la plus grande résolution possible à l'utilisateur, dans le cadre du projet MINERVE. De plus, alors que les paramètres sont optimaux pour une région précise, ils sont ici appliqués à l'ensemble des stations. Nous avons donc une carte complète par modèle (paramétrisation). Le but de cette approche est d'éviter de mélanger des dates sur une même carte, et donc d'amalgamer différentes influences atmosphériques. Les divers modèles présentent toujours une répartition des précipitations cohérente spatialement, avec des tendances claires. Il incombe à l'utilisateur de choisir le modèle approprié pour la région où semble se profiler un événement.
%
%En plus des librairies listées précédemment, le logiciel de visualisation intègre:
%\begin{itemize}
%	\item VroomGIS: une bibliothèque SIG pour l'affichage de données géoréférencées, développé par L. Schreiber au CREALP (\textit{Centre de recherche sur l'environnement alpin});
%	\item wxPlotCtrl: une librairie permettant d'afficher des graphiques interactifs.
%\end{itemize}

% + check annexe D.6

%When working on a region with different parameterizations per subregion, these parameterizations are applied to the wh
