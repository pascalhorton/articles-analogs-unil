...
\begin{itemize}
	...
	\item \textbf{Two-point crossover}: works like the single-point crossover, but there are two intersections defining the segments to exchange. This approach, which significantly extends the search space for the children, is considered more efficient than the previous \citep{Beasley1993b, Haupt2004}.
	...
	\item \textbf{Binary-like crossover} \citep{Haupt2004}: chromosome crossover on a binary coding can generate new values for variables located at intersection points, since the crossovers are applied at the bit level, thus often within a gene. This is not the case for the floating-point representation, since the crossover is performed between the genes. To reproduce the behaviour present in the original algorithms, which introduces new information, \citet{Haupt2004} propose an operator that combines standard crossover with an interpolation approach. The genes located after a crossover point are exchanged, but the gene located at the intersection is modified as follows:
	\begin{equation}
	\left\lbrace \begin{array}{l} 
	g_{e1,n} = g_{p1,n} - \beta (g_{p1,n} - g_{p2,n}) \\
	g_{e2,n} = g_{p2,n} + \beta (g_{p1,n} - g_{p2,n}) \\
	\end{array} \right.
	\label{eq:GA_mating_as_binary}
	\end{equation}
	\noindent
	...
	\item \textbf{Blending method} \citep{Radcliffe1991}: in this approach, instead of exchanging the genes in between the chromosome after a crossover point, these are combined by linear combination.
	\begin{equation}
	\left\lbrace \begin{array}{l} 
	g_{e1,n} = \beta g_{p1,n} + (1-\beta)g_{p2,n} \\ 
	g_{e2,n} = (1-\beta) g_{p1,n} + \beta g_{p2,n} \\
	\end{array} \right.
	\label{eq:GA_mating_blending_method}
	\end{equation}
	\noindent
	...
	%ou $g_{e1,n}$ et $g_{e2,n}$ sont les $n$-ieme gènes des deux enfants, $g_{p1,n}$ et $g_{p2,n}$ ceux des parents, et $\beta$ est un nombre aléatoire entre 0 et 1. Pour $\beta = 0.5$, on obtient la moyenne pour cette variable.
...
\end{itemize}
	...
	\citet{Michalewicz1996} a montré que cette méthode fonctionne bien pour différentes applications. Le choix des variables à mélanger peut être fait de différentes manières, par exemple toutes les variables à droite ou à gauche du point de croisement \citep{Haupt2004}. Les croisements peuvent également être multiples. Les gènes des chromosomes sont alors mélangés en conservant le même $\beta$, ou en le changeant pour chaque gène. Ces méthodes produisent des enfants ayant des variables comprises entre les valeurs des parents, sans sortir de cet intervalle.
	
	\item \textbf{Croisement linéaire} \citep[\textit{linear crossover}]{Wright1991}: afin de permettre aux variables de prendre des valeurs en dehors de l'intervalle défini par les parents, une méthode d'extrapolation est nécessaire. Trois enfants sont alors générés à partir des deux parents, selon l'équation \ref{eq:GA_mating_linear_crossover}. Il y a donc moins de couples sélectionnés pour compléter la population.
	
	\begin{equation}
	\left\lbrace \begin{array}{l} 
	g_{e1,n} = 0.5 g_{p1,n} + 0.5 g_{p2,n} \\ 
	g_{e2,n} = 1.5 g_{p1,n} - 0.5 g_{p2,n} \\ 
	g_{e3,n} = - 0.5 g_{p1,n} + 1.5 g_{p2,n} \\ 
	\end{array} \right.
	\label{eq:GA_mating_linear_crossover}
	\end{equation}
	
	\newpage
	
	\item \textbf{Croisement heuristique} \citep[\textit{heuristic crossover},][]{Michalewicz1996}: le croisement heuristique est une variation de ces dernières méthodes, qui calcule les valeurs des nouveaux gènes selon l'équation suivante:
	
	\begin{equation}
	\left\lbrace \begin{array}{l} 
	g_{e1,n} = \beta (g_{p1,n} - g_{p2,n}) + g_{p1,n} \\
	g_{e2,n} = \beta (g_{p2,n} - g_{p1,n}) + g_{p2,n} \\
	\end{array} \right.
	\label{eq:GA_mating_heuristic_crossover}
	\end{equation}
	
	\item \textbf{Interpolation linéaire} (\textit{linear interpolation}): au contraire des techniques précédentes, nous ne faisons plus de croisement, mais une interpolation linéaire sur tous les gènes du couple. Les enfants sont donc situés sur une ligne connectant les deux parents.
	
	\begin{equation}
	\left\lbrace \begin{array}{l} 
	ind_{e1} = ind_{p1} - \beta (ind_{p1} - ind_{p2}) \\
	ind_{e2} = ind_{p2} + \beta (ind_{p1} - ind_{p2}) \\
	\end{array} \right.
	\label{eq:GA_mating_linear_interpolation}
	\end{equation}
	\noindent
	où $ind_{e1}$ et $ind_{e2}$ sont les individus enfants, $ind_{p1}$ et $ind_{p2}$ les parents, et $\beta$ est une valeur aléatoire entre 0 et 1. $\beta$ est le même pour chaque variable.
	
	\item \textbf{Interpolation libre} (\textit{free interpolation}): cette technique effectue une interpolation sur chaque gène, tout comme la précédente; mais dans ce cas, le facteur de pondération change pour chaque gène:
	
	\begin{equation}
	\left\lbrace \begin{array}{l} 
	ind_{e1} = ind_{p1} - [\beta_{1} (g_{p1,1} - g_{p2,1}), \beta_{2} (g_{p1,2} - g_{p2,2}), ..., \beta_{Ng} (g_{p1,N_{g}} - g_{p2,N_{g}})] \\
	ind_{e2} = ind_{p2} + [\beta_{1} (g_{p1,1} - g_{p2,1}), \beta_{2} (g_{p1,2} - g_{p2,2}), ..., \beta_{Ng} (g_{p1,N_{g}} - g_{p2,N_{g}})] \\
	\end{array} \right.
	\label{eq:GA_mating_free_interpolation}
	\end{equation}
	\noindent
	où $ind_{e1}$ et $ind_{e2}$ sont les individus enfants, $ind_{p1}$ et $ind_{p2}$ les parents, $N_{g}$ le nombre de gènes, et $\beta_{n}$ sont des valeurs aléatoires (entre 0 et 1) indépendantes.
	

De multiples autres méthodes ou variantes existent, combinant les avantages de différentes approches. La performance des variantes étant liée au problème à traiter, nous ne pouvons pas identifier a priori la meilleure technique pour notre étude.
