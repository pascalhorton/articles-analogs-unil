...
To assess the parameters cross-compatibility, those optimized for one subregion were applied to the others. The resulting losses or gains of the CRPSS are displayed in Table \ref{table:crossing_4Zo_calib} for the CP and in Table \ref{table:crossing_4Zo_valid} for the VP.
...
A parametric dependence between the analogy of circulation and the moisture variables could be identified. When the two levels are considered together, the optimal parameters of the circulation analogy are different. This complexity can only be exploited in a suitable manner by global optimization methods.
...
GAs provide parametrizations of AMs that exceed the performance of the sequential calibration. In addition, it as been observed that the forecasts for days with strong precipitation were significantly more improved, which is clearly interesting in the framework of flood forecasting.
